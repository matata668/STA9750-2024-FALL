...
In this project we are going to will investigate the claim that the [US Electoral College](https://en.wikipedia.org/wiki/United_States_Electoral_College) systematically biases election results away from the vox populi. We are going to integrate data from disparate governmental and academic sources,learn to work with spatial data formats, create many plots, and use spatial and animated visualizations to make your argument
...
We have first downloaded Congressional Shapefiles 1976-2012 fully automated using the code shown below.
 ...
 **Data II:** Here is how we downloaded the Congressional Boundry File 1976-2012
...
ggsave("trend_by_state_plot.png", width = 12, height = 14)
...
## Faceted version showing election results over time.
...
```{r include=TRUE, echo=TRUE, warning=FALSE, message=FALSE, results='asis'}
#| code-fold: true
#| code-summary: "Show the code"
...
state_winner_take_all <- function(data, ecv_data) {
  ...
  invisible(state_results) # Suppress display of tibble
}

state_wta_results <- state_winner_take_all(data, ecv_data)

# Writing the state WTA results to a CSV file
...
district_winner_take_all <- function(data, ecv_data) {
  ...
  invisible(combined_results) # Suppress display of tibble
}

district_wta_results <- district_winner_take_all(data, ecv_data)

# Writing the district WTA results to a CSV file
...
state_proportional <- function(data, ecv_data) {
  ...
  invisible(proportional_results) # Suppress display of tibble
}

state_prop_results <- state_proportional(data, ecv_data)

# Writing the state proportional results to a CSV file
...
national_proportional <- function(data, total_ecv = 538) {
  ...
  invisible(national_results) # Suppress display of tibble
}

national_prop_results <- national_proportional(data)

# Writing the national proportional results to a CSV file
...
summary_df <- head(summary_df, 15)

# Display as a table
kable(
  summary_df,
  col.names = c("Year", "Candidate", "District WTA", "National Proportional", "State Proportional", "State WTA"),
  caption = "First 15 Rows of Electoral Vote Allocation Summary"
)
...
# Directory to save PNG files for animation
dir.create("mp03data/election_maps", showWarnings = FALSE)

for (year in years) {
  
  # Filter data for the specific year and find the winner in each state
  winners <- election_data %>%
    filter(year == year) %>%
    group_by(state) %>%
    filter(!writein) %>%
    slice_max(candidatevotes, n = 1) %>%
  ...
  # Merge with electoral votes dataset
  winners <- winners %>%
    left_join(electoral_votes_2000, by = "state_po")
  
  # Prepare and validate shapefile data
  shape_data <- congress_maps[["cd106"]]
  shape_data$STATENAME <- tolower(shape_data$STATENAME)
  winners$state <- tolower(winners$state)
  shape_data <- st_make_valid(shape_data)
  
  # Merge election data with shapefile data
  state_boundaries <- shape_data %>%
    left_join(winners, by = c("STATENAME" = "state")) %>%
    group_by(STATENAME) %>%
    summarise(geometry = st_union(geometry), .groups = "drop") %>%
    left_join(winners, by = c("STATENAME" = "state"))
  ...
  # Main map with insets and annotations
  election_map <- ggplot() +
    geom_sf(data = continental_states, aes(fill = party_simplified), color = "white", size = 0.5) +
    scale_fill_manual(values = c("DEMOCRAT" = "blue", "REPUBLICAN" = "red")) +
    labs(title = paste(year, "Presidential Election Results by State"), fill = "Winning Party") +
    theme_minimal() +
    theme(axis.text = element_blank(),
          axis.ticks = element_blank(),
          axis.title = element_blank(),
          panel.grid = element_blank()) +
    coord_sf(xlim = c(-130, -60), ylim = c(24, 50), expand = FALSE) +
    annotation_custom(ggplotGrob(ggplot() + geom_sf(data = alaska, aes(fill = party_simplified), color = "white") + theme_void()), xmin = -125, xmax = -105, ymin = 25, ymax = 35) +
    annotation_custom(ggplotGrob(ggplot() + geom_sf(data = hawaii, aes(fill = party_simplified), color = "white") + theme_void()), xmin = -104, xmax = -85, ymin = 25, ymax = 32)
  
  # Add electoral votes as labels
  election_map <- election_map +
    geom_text(data = continental_states %>% filter(!state_po %in% northeast_states),
              aes(label = electoralvotes, geometry = geometry), 
              stat = "sf_coordinates", size = 3, color = "white") +
  ...
  # Save each year’s map as PNG
  ggsave(filename = paste0("mp03data/election_maps/election_map_", year, ".png"),
         plot = election_map, width = 20, height = 15)
}

# Load all images for GIF creation
